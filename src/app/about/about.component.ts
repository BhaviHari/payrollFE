    ...
    this.dataService.getMenuUrl()
      .subscribe((results: any) => {
        ...
        if (data) {
          ...
          //   return true;
          // }
          const returnUrlString = localStorage.getItem('returnURL');

          if (returnUrlString !== null) {
            const returnUrl = JSON.parse(returnUrlString);
            if (returnUrl) {
              this.router.navigateByUrl(returnUrl);
              localStorage.removeItem("returnURL");
              // return true;
          ...
          // Handle the case when 'returnURL' is not present in localStorage or cannot be parsed
