...
export class NewPayStructuresComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.empdetails.get('monthRemun')!.valueChanges.subscribe((value: any) => {
    ...
    this.empdetails.get('monthSegA')!.valueChanges.subscribe((val : any) => {
      const cyrSegA = val * 12;
      this.empdetails.get('yrSegA')!.setValue(cyrSegA);
      this.calculateMonthSpl();
    })
    this.empdetails.get('monthSegB')!.valueChanges.subscribe((val : any) => {
      const cyrSegB = val * 12;
      this.empdetails.get('yrSegB')!.setValue(cyrSegB);
      this.calculateMonthSpl();
    })
    this.empdetails.get('monthSegC')!.valueChanges.subscribe((val : any) => {
      const cyrSegC = val * 12;
      this.empdetails.get('yrSegC')!.setValue(cyrSegC);
      this.calculateMonthSpl();
    })
    // this.empdetails.get('yrSegX').valueChanges.subscribe((val : any) => {
    //   this.calculateMonthSpl();
    // })
    this.empdetails.get('basicpercnt')!.valueChanges.subscribe((vals: any) => {
      this.calculateMonthBasic();
      this.calculatePercent();
    })
    this.empdetails.get('allowpercnt')!.valueChanges.subscribe((vals: any) => {
      this.calculateMonthAllow();
    })
    this.empdetails.get('monthD')!.valueChanges.subscribe((values: any) => {
    ...
    this.empdetails.get('monthSegE')!.valueChanges.subscribe((vals: any) => {
      let yrCCv = vals * 12;
      this.empdetails.get('monthCC')!.setValue(vals);
      this.empdetails.get('yrCC')!.setValue(yrCCv);
      this.calculateCTC();
    })
    this.empdetails.get('monthtakeHome')!.valueChanges.subscribe((vals: any)=>{
      let yrtake = vals * 12;
      this.empdetails.get('yrtakeHome')!.setValue(yrtake);
      this.calculateAndStoreGrossSal();
    })
    this.empdetails.get('monthGross')!.valueChanges.subscribe((val : any)=>{
    ...
    this.empdetails.get('monthCCPF')!.valueChanges.subscribe((val : any)=>{
      this.calculateCompContrib();
      // this.calculateTotalDed();
    })
    this.empdetails.get('monthCCESI')!.valueChanges.subscribe((val : any)=>{
      this.calculateCompContrib();
      // this.calculateTotalDed();
    })
    this.empdetails.get('stmonthPF')!.valueChanges.subscribe((val : any)=>{
      this.calculateStatury();
      // this.calculateTotalDed();
    })
    this.empdetails.get('stmonthESI')!.valueChanges.subscribe((val : any)=>{
      this.calculateStatury();
      // this.calculateTotalDed();
    })
    ...
    this.segmentData.Earnings.forEach((segment: any) => {
  ...
  processApiResponse(): void {
    this.paycomponentData.forEach((item: any) => {
  ...
  getMonthFormControlName(segment : any): string {
    return `month${segment.paycomponent_name}`;
  }

  getYearFormControlName(segment : any): string {
