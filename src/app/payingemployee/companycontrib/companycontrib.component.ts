...
export class CompanycontribComponent implements OnInit {
  ...
  public allcompanylist!: icompanyList[]; 
    ...
    public chipSelectedcompanyid : any = [];
    public chipRemovedcompanyid : any = [];
    ...
    public chipeditSelectedcompanyid: any = [];
    public chipeditRemovedcompanyid: any = [];
...
console.log('dat', data)
    ...
    this.payrollService.getCompany_ParticularGet(data.id).subscribe(res => {
      const extractedId = res.paycomponentflagmaster.map((item: any) => item.id);
  ...
  public selectcompanyByName(companyName: any) {
  ...
  onEdit() {
    ...
    // const selectedIds = selectedValues.map((type: any) => type.id);
      ...
      const refIds = selectedValues.map((ids: any) => ({ "ref_id": ids.id }));
  ...
  public editselectcompanyByName(companyName: any) {
