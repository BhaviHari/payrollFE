...
export class TemplatemasterComponent implements OnInit {
  ...
  @ViewChild('leveldata')matinputAutocomplete: MatAutocomplete | any;
  @ViewChild('component') matcomponentAutocomplete: MatAutocomplete | any;
  @ViewChild('getcomponentInput') getcomponentInput: any;
  @ViewChild('editcomponent') mateditcomponentAutocomplete: MatAutocomplete | any;
...
has_previous = true;
    ...
    public chipSelectedsegmentid : any= [];
    public chipRemovedsegmentid: any = [];

    public segmentdata!: iseditsegmentlist[];
    public chipeditselectedsegment: iseditsegmentlist[] = [];
    public chipeditselectedsegmentid : any= [];
    public chipeditRemovedsegmentid: any = [];
...
@ViewChild('editsegmentInput') editsegmentInput: any;
  @ViewChild('auto')
  editmatAutocomplete!: MatAutocomplete;
    gradeList: any;
  ...
  newSegments()
    ...
    editSegment(datas: any) {
      ...
      this.payrollService.getTemplateParticular(datas.id).subscribe(res => {
        let datam = res['data'];
        const extractedId = datas.segment.map((segment: any) => segment.name);
      ...
      let segmentNames = datas.segment.map((segment:any) => segment.name);
    }
    ...
    private getFunctional(gstkeyvalue:any) {
    ...
    public displayfncomponent(component?: any): string | any {
    ...
    getComponentgrade(gradelevel: any) {
    ...
    onSubmitAdd(){
      ...
      const refIds = selectedValues.map((id: any) => ({ "ref_id": id.id}));
      let  datapay = {
          "name": data.name,
  ...
  public selectsegmentByName(segmentName : any) {
    let foundsegment1 = this.chipSelectedsegment.filter((segment: any) => segment.name == segmentName);
    if (foundsegment1.length) {
      return;
    }
    let foundsegment = this.allsegmentlist.filter((segment : any) => segment.name == segmentName);
...
public editremovedsegment(segment: iseditsegmentlist): void {
  ...
  public editselectsegmentByName(segmentName: any) {
  ...
  fileChange(file: any) {
    let files : FileList = file.target.files;
