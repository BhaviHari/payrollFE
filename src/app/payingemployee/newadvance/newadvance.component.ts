...
export class NewadvanceComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const getDataid : any = localStorage.getItem("sessionData");
  ...
  chosenYearHandler(normalizedYear: Moment) {
    const ctrlValue : any= this.from_date.value;
  ...
  chosenYearHandlers(normalizedYear: Moment) {
    const ctrlValues: any = this.payableMonth.value;
  ...
  chosenYearHandle(normalizedYear: Moment) {
    const ctrlValues: any = this.to_date.value;
    ctrlValues.year(normalizedYear.year());
  ...
  chosenYearHandl(normalizedYear: Moment) {
    const ctrlValues : any= this.payablesMonth.value;
    ctrlValues.year(normalizedYear.year());
  ...
  editAdvance(data: any) {
    this.payRollService.getAdvanceParticularGet(data.id).subscribe((result) => {
      ...
      const formattedDates : any = this.datepipe.transform(
        result.to_date,
        "yyyy-MM"
      );
  ...
  searchFunctionalHead(name: any, status: any) {
  ...
  searchFunctionalHeads(name: any, status: any) {
  ...
  searchfunctionchecker(name: any, status: any) {
  ...
  advanceEdit() { }
  countApprovedRows(apiResponse: any) {
  ...
  countPendingRows(apiResponse: any) {
    for (const item of apiResponse) {
      if (
        item.advance_status &&
  ...
  countRejectedRows(apiResponse: any) {
    for (const item of apiResponse) {
  ...
  chosenYearHandlere(normalizedYear: Moment) {
    const ctrlValue : any = this.from_dates.value;
    ctrlValue.year(normalizedYear.year());
  ...
  ApproveViews(data: any) {
    const checkdraft : any= document.querySelector('.checkdraft')
    if (data == 'DRAFT') {
  ...
  chosenYearHandlet(normalizedYear: Moment) {
    const ctrlValue : any = this.to_dates.value;
    ctrlValue.year(normalizedYear.year());
