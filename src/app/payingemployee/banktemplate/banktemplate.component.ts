...
uploadForms: FormGroup | any;
  ...
  clear(){
    ...
    this.SelectedSearchId = null as any;
    this.Summary(this.pagination.index)
  }
  bank(){
  ...
  remove(chip : any): void {
    ...
    const index = this.FieldChiplist.findIndex((chip: any) => chip === chip); 
  ...
  NameSelected(event:MatAutocompleteSelectedEvent){
    ...
    let found=this.FieldChiplist.filter((event: any)=>event.name==events)
    let foundPush=this.fieldDropdown.filter((event: any)=>event.name==events)
    if(found.length){
      return
    }
    if(foundPush.length){
  ...
  NameSelectedEdit(event:MatAutocompleteSelectedEvent){
    ...
    let events=event.option.value.name
    let id=event.option.value.id
    let found=this.FieldChiplistEdit.filter((event: any)=>event.name==events)
    let foundPush=this.fieldDropdown.filter((event: any)=>event.name==events)
    if(found.length){
      return
    }
  ...
  public displayfunc(fn?:FunctionHead):string | any{
  ...
  submit(){
    if(this.templateCreate.value.bankname=='' || this.templateCreate.value.bankname==null){
      ...
      const columnId=this.FieldChiplist.map((id: any,index: any)=>({'column_id':id.column_id,'order':index+1}))
    const payload={
      name:'',
  ...
  Update(){
    if(this.templateEdit.value.bankname=='' || this.templateEdit.value.bankname==null){
      ...
      this.columnId=this.FieldChiplistEdit.map((id: any,index: any)=>({'column_id':id.column_id,'order':index+1,'id':id?.id}))
  ...
  const a=this.FieldChiplistEdit.map((id: any)=>id.column_id)
  this.templateEdit.get('fields')?.setValue(a)
  const refId=this.FieldChiplistEdit.map((id: any)=>({'column_id':id.column_id}))
    this.payrollService.EditBank(data.id).subscribe(result=>{
      this.SelectedId=result.bank?.id
      const s=result.details.map((id: any)=>({'column_id':id.column_id}))
