...
export class PfcategoryComponent implements OnInit {
  ...
  public paycomponent_array: any=[]; 
  public chipSelectedemployee: iemployeeList[] = [];
  public chipSelectedemployeeid: any = [];
  ...
  filterOptions(value: string): any {
    ...
    return this.paycomponent_array.filter((option: any) => option.name.toLowerCase().includes(filterValue));
  }
      // paycomponentflagmaster: [[{}]],
  // control=this.PFForm.get('paycomponentflagmaster')

  get_pFSummary(pageno: number) {
  ...
  addEmployeePF(formtype: any, data: any) {
    ...
    if (data != '') {
      ...
      this.payrollservice.getPF_ParticularGet(data.id)
        ...
        .subscribe((res) => {
          ...
          const extractedId = res.paycomponentflagmaster.map((item: { id: any; }) => item.id);
          console.log(res)
  ...
  pfSubmit() {
    ...
    // }
      ...
      const refIds = selectedValues.map((ids: { id: any; }) => ({ "ref_id": ids.id}));
  ...
  onSelectionChange($event: { isUserInput: boolean; source: { _selected: boolean; value: any; }; }) {
  ...
  public selectemployeeByName(employeeName: any) {
    let foundcompany1 = this.chipSelectedemployee.filter(employee => employee.name == employeeName);
    if (foundcompany1.length) {
      return;
    }
    let foundcompany = this.paycomponent_array.filter((employee: any) => employee.name == employeeName);
