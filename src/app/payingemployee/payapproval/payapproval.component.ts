...
export class PayapprovalComponent implements OnInit {
  ...
  @ViewChild('fn')
  matFHAutocomplete!: MatAutocomplete;
  ...
  searchData() {
    ...
    if (this.splAllowance == false) {
      ...
      const selectedMonthYears : any= this.monyear.value; // Assuming this contains the selected month and year
  ...
  filterGroupedData(groupedData: any, draftsList: number[]): any {
    return groupedData.map((group: any) => {
      return group.filter((item: any) => draftsList.includes(item.id));
  ...
  onSingleChecked(event: any, row: any) {
    if (event.target.checked) {
  ...
  chosenYearHandlers(normalizedYear: Moment) {
    const ctrlValue: any = this.monyears.value;
    ctrlValue.year(normalizedYear.year());
  ...
  employeecomponent_search(value: any) {
    ...
    this.apiservice.getFunctionalHead(values, 1).subscribe(data => {
