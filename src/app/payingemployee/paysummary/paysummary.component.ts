  ...
  searchData() {
    ...
    this.apiservice.getPaystmtSummarySearch(this.pagination.index, this.send_value).subscribe(results => {
      ...
      if (results['data'] && results['data'].length === 0) {
        ...
        let allKeys : any = Array.from(new Set(apiData.flatMap((item: any) => Object.keys(item))));   
        let staticColumns = [
            "Name",
            "Code",
            "PF Number",
        ...
        this.displayedColumns = staticColumns.concat(allKeys.filter((key:any) => !staticColumns.includes(key)));
