...
export class EmpdetailViewComponent implements OnInit {
  ...
  submit_tochecker_approve(statuskey: any){
  ...
  download_click(){
    ...
    const pdf : any=document.querySelector('.pdfDownload');
    pdf.classList.add('fa-bounce');
  }
  closes(){
    const StyleClose: any=document.querySelector('.pdfDownload')
...
RouteToEmployeeCreate(data: any){
  let obj = {
    routes: true,
    data: data,
    empid: this.emp_id 
