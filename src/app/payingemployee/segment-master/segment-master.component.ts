...
export class SegmentMasterComponent implements OnInit {
  @ViewChild('segment') matcomponentAutocomplete: MatAutocomplete | any;
  ...
  @ViewChild('Bonus') matbonusAutocomplete: MatAutocomplete | any;
  ...
  getComponentsegment(component_type: any) {
    let values = component_type.target.value
    this.payrollService.getsegmentpageApi(values,1).subscribe(data => {
      this.comp_type_data = data['data'];
    });
  }

  getComponentbonus(type: any){
  ...
  newSegments()
    ...
    editSegment(datas: { name: any; percentage: any; id: any; glno: any; type: { name: any; id: any; }; component_type: { name: any; id: any; }; })
    { 
        this.isSummaryContent = false;
    ...
    getFunctionalbonus(gstkeyvalue: any) {
      let values = gstkeyvalue.target.value
      this.payrollService
        .deductiontypeApi(values, 1)
        .subscribe((results: any) => {
          let datas = results['data'];
          this.deduction_type_data = datas;
        });
...
getFunctional(gstkeyvalue: any) {
  let values = gstkeyvalue.target.value;
      this.payrollService
        .getsegmentpageApi(values, 1)
        .subscribe((results: any) => {
          let datas = results['data'];
          this.comp_type_data = datas;
        });
  ...
  public displayfnsegment(segment?: any): string | any {
    ...
    setTimeout(() => {
      if (
        ...
        fromEvent(this.matcomponentAutocomplete.panel.nativeElement, 'scroll')
          .pipe(
            map((x: any) => (x.target as HTMLElement).scrollTop),
  ...
  public displayfnBonus(Bonus?: any): string | any {
