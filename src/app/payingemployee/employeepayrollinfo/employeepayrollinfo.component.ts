...
export class EmployeepayrollinfoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.subscription = this.emp_paysrtuctureform.get('employeepay_detail')!.valueChanges.subscribe(data => {
      this.total = data.reduce((a: any, b: any) => a + +b.paycomponent_percentage, 0).toFixed(2);
      this.amount_total = data.reduce((a: any, b: any) => a + +b.amount, 0)     
    })
...
this.emp_paysrtuctureform.get('employeepay_detail')?.valueChanges.subscribe(value_arryas => {
  ...
  // getpfstrutureinfo
  paystructure_infoget(emp_id: any) {
  ...
  formdatabinding(value: any, emp_id: any) {
  ...
  deduction_binding(value: any, emp_id: any) {
  ...
  amount_calculation(pay_compercentage: any, ctc: any, is_monthly: any) {
  ...
  paycomponent_search(value: any) {
    ...
    this.apiservice.getpaycomponents(values, 1).subscribe(data => {
      this.paycomponent_array = data['data'];
    });
  }
  paytype_search(value: any) {
  ...
  isCustomTypeSelected(index: number): boolean {
    const deductionDataArray = this.deductionform.get('deduction_data') as FormArray;
    const typeControl : any = deductionDataArray.at(index).get('type');
    return typeControl && typeControl.value === 5;
  }
  ...
  paycomponent_searchs(value: any) {
    let values = value.target.value;
    this.apiservice.searchcompcontrib(values, 1).subscribe(data => {
      this.paycomponent_arrays = data['data'];
    });
  }
  ...
  compdatabinding(value  : any, emp_id : any) {
  ...
  type_infos(type_info: any,index: any) {
  ...
  getSelectedOption(event: MatAutocompleteSelectedEvent, index: any) {
  ...
  type_infob(type_info: any, index: any) {
  ...
  bonus_binding(value: any, emp_id: any) {
  ...
  getSelectedOptions(event: MatAutocompleteSelectedEvent, index: any) {
    console.log('Selected option:', event.option.value);
    // const percentageValue = event.option.value.percentage.replace('%', '');
    // this.selectedPercent = percentageValue;
    // console.log("Selected Percent", this.selectedPercent)
