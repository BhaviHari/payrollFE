...
export class YearlysummaryComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('fn')
  matFHAutocomplete: MatAutocomplete | any;
  ...
  ngOnInit(): void {
    ...
    if (this.payRole) {
      let isAdmin = false;
      this.payRole.forEach((element : any) => {
  ...
  loadData() {
    ...
    if (!this.yearlyForm.value.year) {
      ...
      this.payRollService.getYearlyPay(this.empId, yr).subscribe((results: MonthlyPayData) => {
        ...
        this.payrollDates = transformedJSON.pay_info_month.map((item: { payroll_date: any; }) => item.payroll_date);
        this.monthlyPayData = transformedJSON.pay_info_month.map((item: { Employeemonthlypay_details_data: any; }) => item.Employeemonthlypay_details_data);
        ...
        this.dataSource = payComponent.map((component: any) => {
          const row: any = {
            item: component,
          };
          ...
          this.payrollDates.forEach((date, index) => {
            ...
            const payData = monthlyPay.find((pay: { paycomponent: any; }) => pay.paycomponent === component);
